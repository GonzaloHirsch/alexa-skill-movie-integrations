...
const AskRandomMovieIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        let foundMovie = false, currentTry = 0, randomMovieId = 0, movieResponse = null;
        while (!foundMovie && currentTry <= 5) {
            ...
            console.log(`Try ${currentTry} for movie ${randomMovieId}. Got ${util.inspect(movieResponse, true, null, false)}`);
        ...
        if (!foundMovie) {
            ...
            .speak(`Sorry, I couldn't find a suitable movie right now. Please try again later.`)
            .reprompt(`Sorry, I couldn't find a suitable movie right now. Please try again later.`)
            .getResponse();
        }
